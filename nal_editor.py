...
class NALEditor:
    ...
    def __init__(self, **kwargs):
        ...
        if os.path.exists("./res/doc.enc"):
            ...
            showinfo(
                "Fatal Error", "File '%s' Is Missing From Installation Folder" % "doc.enc")
            exit()
        ...
        try:
            ...
            showinfo(
                "Fatal Error", "File '%s' Is Missing From Installation Folder" % "nal_opcodes.json")
            exit()
        ...
        self.__thisHelpMenu.add_command(
            label="About NAL Editor", command=self.__showAbout)
        self.__thisCommandMenu.add_command(
            label="Opcodes", command=self.__showOpcodes)
        self.__thisCommandMenu.add_command(
            label="Compile", command=self.__runCompiler)
    ...
    def __openFile(self):
        self.__file = askopenfilename(defaultextension=".nal", filetypes=[
                                      ("Nano Files", "*.nal*"), ("All Files", "*.*")])
        if self.__file == "":
            self.__file = None
        else:
    ...
    def __saveFile(self):
        if self.__file == None:
            # save as new file
            self.__file = asksaveasfilename(initialfile="Untitled.nal", defaultextension=".nal", filetypes=[
                                            ("Nano Files", "*.nal*"), ("All Files", "*.*")])
        if self.__file == "":
            self.__file = None
        else:
    ...
    def __runCompiler(self):
        if self.__file == None:
            # save as new file
            self.__file = asksaveasfilename(initialfile="Untitled.nal", defaultextension=".nan", filetypes=[
                                            ("All Files", "*.*"), ("Nano Files", "*.nal*")])
        if self.__file == "":
            self.__file = None
        ...
        try:
            ...
            showinfo(
                "Compiler Error", "Max No. Of Instructions Allowed Exceeded\nCan't Compile More Than 256 Instructions")
        ...
        try:
            NALEditor.compiler.compile(
                codes, NALEditor.compiler.opcodes, nac_file)
            showinfo("Success", "Successfully Compiled 1 File\n At '%s'" % nac_file)
        except:
            showinfo("Compiler Error", "Unrecognized Instruction -> %s" %
                     NALEditor.compiler.error)
            raise Exception("Error: Compilation Failed")
